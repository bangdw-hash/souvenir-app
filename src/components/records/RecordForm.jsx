...
export default function RecordForm({ groupId, patients, appointments, initial, onSuccess, onCancel }) {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      // 다음 예약일이 새로 입력됐거나 변경된 경우 자동으로 일정 생성
      const prevNextVisitDate = initial?.nextVisitDate || '';
      if (form.nextVisitDate && form.nextVisitDate !== prevNextVisitDate) {
        const linkedAppt = appointments.find((a) => a.id === form.apptId);
        await addAppointment(groupId, {
          patientId: form.patientId,
          hospital: linkedAppt?.hospital || '',
          dept: linkedAppt?.dept || '',
          doctor: linkedAppt?.doctor || '',
          date: form.nextVisitDate,
          time: form.nextVisitTime || '',
          status: '예약완료',
          note: '진료 기록에서 자동 등록된 예약',
        });
      }
        ...
        <div>
          ...
          {form.nextVisitDate && (
            <p className="text-xs text-blue-500 mt-1.5 flex items-center gap-1">
              <span>📅</span>
              <span>
                저장 시 <strong>{form.nextVisitDate}{form.nextVisitTime ? ` ${form.nextVisitTime}` : ''}</strong> 일정이 자동 등록됩니다
              </span>
            </p>
          )}
        </div>
              ...
              {newFiles.map((f, i) => (
                <div key={i} className="flex items-center gap-2 bg-blue-50 rounded-lg px-3 py-2 text-xs text-gray-600">
                  ...
                  {isUploading && uploadProgress[i] !== undefined ? (
                    ...
                    <button type="button" onClick={() => removeNewFile(i)} className="text-gray-400 hover:text-red-400"><X size={13} /></button>
                  )}
                </div>
              ))}
            </div>
        ...
        <div className="fixed inset-0 z-50" onClick={() => setShowFilePicker(false)}>
          ...
          <div className="absolute bottom-0 left-0 right-0 bg-white rounded-t-3xl p-5 space-y-3 safe-area-pb" onClick={(e) => e.stopPropagation()}>
            ...
            <button onClick={() => cameraRef.current?.click()}
              ...
              <div className="w-11 h-11 bg-blue-100 rounded-xl flex items-center justify-center flex-shrink-0"><Camera size={22} className="text-blue-600" /></div>
              <div><p className="text-sm font-semibold text-gray-900">카메라 촬영</p><p className="text-xs text-gray-400 mt-0.5">지금 바로 촬영하여 첨부</p></div>
            </button>
            <button onClick={() => galleryRef.current?.click()}
              className="w-full flex items-center gap-4 p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 active:bg-gray-200 transition-colors text-left">
              <div className="w-11 h-11 bg-green-100 rounded-xl flex items-center justify-center flex-shrink-0"><Image size={22} className="text-green-600" /></div>
              <div><p className="text-sm font-semibold text-gray-900">갤러리에서 선택</p><p className="text-xs text-gray-400 mt-0.5">사진 및 동영상 앨범에서 선택</p></div>
            </button>
            <button onClick={() => fileRef.current?.click()}
              className="w-full flex items-center gap-4 p-4 bg-gray-50 rounded-2xl hover:bg-gray-100 active:bg-gray-200 transition-colors text-left">
              <div className="w-11 h-11 bg-purple-100 rounded-xl flex items-center justify-center flex-shrink-0"><Folder size={22} className="text-purple-600" /></div>
              <div><p className="text-sm font-semibold text-gray-900">파일 업로드</p><p className="text-xs text-gray-400 mt-0.5">PDF, 문서, 이미지 파일 선택</p></div>
            </button>
            <button onClick={() => setShowFilePicker(false)} className="w-full py-3 text-sm text-gray-400 hover:text-gray-600 mt-1">취소</button>
          </div>
        </div>
      )}
    </>
