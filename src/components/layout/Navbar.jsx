...
const navItems = [
  ...
  { to: '/calendar', icon: Calendar, label: '캐린더' },
          ...
          return (
            <Link
              ...
              className={`flex flex-col items-center gap-0.5 py-2 px-4 min-w-[60px] transition-colors
                ${highlight ? 'relative' : isActive ? 'text-blue-600' : 'text-gray-400 hover:text-gray-600'}`}
            >
              {highlight ? (
              ...
              <span className={`text-[10px] font-medium ${highlight ? 'text-blue-500' : ''}`}>{label}</span>
            </Link>
          );
        })}
      </div>
