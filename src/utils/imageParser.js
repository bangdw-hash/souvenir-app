...
function extractDatetime(block) {
  const full = block.match(
    /(\d{4})\.(\d{1,2})\.(\d{1,2})\s*[\(（]?[월화수목금토일][\)）]?\s*(\d{1,2})[:시]\s*(\d{2})/
  );
  if (full) {
    return {
      date: `${full[1]}-${full[2].padStart(2, '0')}-${full[3].padStart(2, '0')}`,
      time: `${full[4].padStart(2, '0')}:${full[5]}`,
    };
  }
  const dateOnly = block.match(
    /(\d{4})\.(\d{1,2})\.(\d{1,2})\s*[\(（]?[월화수목금토일][\)）]?/
  );
  if (dateOnly) {
    const timeM = block.match(/(\d{1,2})[:시]\s*(\d{2})/);
    return {
      date: `${dateOnly[1]}-${dateOnly[2].padStart(2, '0')}-${dateOnly[3].padStart(2, '0')}`,
...
function extractDoctorDept(block) {
  ...
  const deptFirst = block.match(
    /([가-힣]{2,10}(?:내과|외과|이비인후과|안과|피부과|정형외과|신경외과|비뇨의학과|산부인과|소아청소년과|흉부외과|신경과|성형외과|재활의학과|마취통증의학과|영상의학과|응급의학과|정신건강의학과|방사선종양학과|혈액종양내과|소화기내과|순환기내과|호흡기내과|내분비내과|신장내과|감염내과|류마티스내과))\s+([가-힣]{2,4})/
  );
  if (deptFirst) return { doctor: deptFirst[2], dept: deptFirst[1] };

  const deptOnly = block.match(
    /([가-힣]{2,10}(?:내과|외과|이비인후과|안과|피부과|정형외과|신경외과|비뇨의학과|산부인과|소아청소년과|흉부외과|신경과|성형외과|재활의학과))/
  );
...
function extractNote(block) {
  ...
  if (specialty) {
    const cleaned = specialty[1].replace(/\n/g, ' ').replace(/\s+/g, ' ').trim();
    if (cleaned) parts.push(`분야: ${cleaned}`);
  }
